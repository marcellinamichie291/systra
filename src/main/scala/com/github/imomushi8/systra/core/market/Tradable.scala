...
trait Tradable[Market](using MarketBehavior[Market]):
  ...
  private def execAction[Memory](chart: Chart,
                                 brain: Brain[Market, Memory]): StateT[IO, (Market, Memory, TradeState), TradeAction[Market, Memory]] =
    StateT.inspectF {
      case (market, memory, state) =>
        state.flag match
  ...
  private def updateFlag[Memory](action: TradeAction[Market, Memory]): StateT[IO, (Market, Memory, TradeState), Unit] =
    StateT.modify {
  ...
  private def updateTradeState[Memory](transactions: Vector[PositionTransaction]): StateT[IO, (Market, Memory, TradeState), Unit] = StateT.modify {
    case (currentMarket, currentMemory, currentState) =>
      val init = (
        currentState.buyRecord,
        currentState.sellRecord,
        currentState.capital,
        currentState.maxCapital,
        currentState.consecutiveWins,
        currentState.consecutiveLoses)

      val sortedTransactions = transactions.sortBy(_.openTime)
      val (buyRecord, sellRecord, capital, maxCapital, consWin, consLose) =
        sortedTransactions.foldLeft(init) {
      ...
      val nextState = TradeState(
        currentState.sampleSize,
        currentState.flag,
        buyRecord,
        sellRecord,
        capital,
