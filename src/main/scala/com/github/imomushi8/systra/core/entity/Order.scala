 ...
 *   方向（BUY:1, SELL:-1）
 * @param price
 *   指値価格（LIMIT,STOP_LIMITの場合必須、それ以外は0）
 * @param triggerPrice
 *   逆指値価格（STOP,STOP_LIMITの場合必須、それ以外は0）
 * @param size
 *   数量
 * @param expire
 *   有効期限
 * @param settlePositionId
 *   決済する場合のポジションID（未指定の場合 Monoid[ID].empty を利用）
 * @param parentId
 *   親注文のID（デフォルトでMonoid[ID].empty）
 * @param brotherId
 *   OCO注文のID（デフォルトでMonoid[ID].empty）
 */
case class Order(id               :ID,
  ...
  override lazy val toString: String =
    val ifdoco          =
      if hasParent && hasBrother then "IFDOCO " else if hasParent then "IFD " else if hasBrother then "OCO " else ""
